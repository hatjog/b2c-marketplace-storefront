...
function mapPayloadPageToBlogPost(page: PayloadPage, index: number): BlogPost {
  ...
  const excerpt =
    page.excerpt ||
    page.summary ||
    "Read the latest updates from our marketplace blog."
...
export async function fetchHomepageProducts({
  ...
  url.searchParams.set(
    "fields",
    "*variants.calculated_price,*variants,*seller"
  )
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `[homepage][products] fetch failed: ${response.status} ${response.statusText}`
      )
      return []
    }
    ...
    if (products.length === 0) {
      ...
      if (fallbackResponse.ok) {
        ...
        products = Array.isArray(fallbackData.products)
...
export async function fetchHomepageBlogPosts({
  ...
  const normalizedBase = payloadApiUrl.endsWith("/")
    ? payloadApiUrl
    : `${payloadApiUrl}/`
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `[homepage][blog] fetch failed: ${response.status} ${response.statusText}`
      )
      return []
    }
